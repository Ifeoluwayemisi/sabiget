...
async function testNearbyVendors() {
  ...
  try {
    const response = await request('GET', '/api/v1/customers/nearby-vendors?latitude=6.5244&longitude=3.3792&radius=5');
    ...
    if (response.status === 200) {
      ...
      if (response.body.vendors?.length > 0) {
        ...
        log(`  First vendor: ${vendor.name} (${vendor.distanceKm}km away)`, 'yellow');
        log(`  Rating: ${vendor.averageRating}⭐ (${vendor.totalReviews} reviews)`, 'yellow');
      }
    } else {
...
async function testOrderHistory() {
  ...
  try {
    const response = await request('GET', '/api/v1/customers/order-history?page=1&limit=5');
    ...
    if (response.status === 200) {
      ...
      if (response.body.pagination) {
        log(`  Pagination: Page ${response.body.pagination.page}/${response.body.pagination.totalPages}`, 'yellow');
      ...
      if (response.body.orders?.length > 0) {
        ...
        log(`  Amount: ₦${order.totalAmount}, Vendor: ${order.vendor.name}`, 'yellow');
        if (order.rating) {
...
async function testLoyaltyPoints() {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      log(`  Earning rate: ${(points.earningRate * 100).toFixed(0)}%`, 'yellow');

      if (points.nextTier) {
        log(`  Next tier: ${points.nextTier.name} (${points.nextTier.ordersNeeded - points.nextTier.ordersCompleted} orders remaining)`, 'yellow');
      }
    } else {
      log(`✗ Unexpected status: ${response.status}`, 'red');
    }
...
async function testCustomerInsights() {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      log(`  This month: ${insights.recentOrdersThisMonth} orders (${insights.frequencyPerWeek}/week)`, 'yellow');
...
async function testRecommendations() {
  ...
  try {
    const response = await request('GET', '/api/v1/customers/recommendations?latitude=6.5244&longitude=3.3792&radius=5');

    if (response.status === 200) {
      log('✓ Status 200 OK', 'green');
      log(`✓ Found ${response.body.recommendations?.length || 0} recommendations`, 'green');

      response.body.recommendations?.forEach((vendor, index) => {
        log(`  ${index + 1}. ${vendor.name} (${vendor.distanceKm}km)`, 'yellow');
        log(`     Rating: ${vendor.rating}⭐ | Favorite: ${vendor.isFavorite ? 'Yes' : 'No'} | Reason: ${vendor.reason}`, 'yellow');
      });
    } else {
      log(`✗ Unexpected status: ${response.status}`, 'red');
    }
...
async function testAll() {
  log(`\n${colors.bold}=== Phase 3 Customer Experience Tests ===${colors.reset}`, 'blue');
  log(`API URL: ${API_URL}`, 'yellow');
  log(`Token: ${ACCESS_TOKEN ? 'Set ✓' : 'Not set ✗'}`, ACCESS_TOKEN ? 'green' : 'red');
